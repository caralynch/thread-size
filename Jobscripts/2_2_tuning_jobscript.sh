...
if [ -z "${1-}" ]; then
    ...
    echo "Usage: qsub 2_2_tuning_jobscript.sh <subreddit>"
    exit 1
fi
...
# Output directory
OUTDIR="/home/ucabcpl/Scratch/thread_size/thread-size/Outputs/2_Thread_Size/${SUBREDDIT}/2_tuning"
mkdir -p "${OUTDIR}"
...
run_step "${SUBREDDIT}_2_tuning" \
  "${SCRIPT_DIR}/2_tuning.py" \
  --subreddit "${SUBREDDIT}" \
  --outdir "${OUTDIR}" \
  --train_X "${TRAIN_X}" \
  --train_y "${TRAIN_Y}" \
