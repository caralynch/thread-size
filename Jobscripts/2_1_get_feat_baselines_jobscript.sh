...
if [ -z "${1-}" ]; then
    ...
    echo "Usage: qsub 2_1_get_feat_baselines_jobscript.sh <subreddit>"
    exit 1
fi
...
# Output directory
OUTDIR="/home/ucabcpl/Scratch/thread_size/thread-size/Outputs/2_thread_size/${SUBREDDIT}/1_feat_baselines"
mkdir -p "${OUTDIR}"
...
run_step "${SUBREDDIT}_2_feature_baselines" \
  "${SCRIPT_DIR}/1_feature_baselines.py" \
  --subreddit "${SUBREDDIT}" \
  --outdir "${OUTDIR}" \
  --train_X "${TRAIN_X}" \
  --train_y "${TRAIN_Y}" \
