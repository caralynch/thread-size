...
COLS_TO_REMOVE = ['body', 'subject', 'success', 'thread_id', 'author', 'domain', 'clean_text', 'domain_type']
TARGET_Y_COLS = ['timestamp','thread_size', 'authors', 'score', 'thread_depth', 'direct_reply_count', 'direct_reply_sentiment_std', 'mean_direct_reply_length']

# Order in which to handle correlated pairs. Lower indices indicate higher priority for keeping column.
PRIORITY_ORDER = ["author_freq", "domain_freq", 'encoded_author', 'encoded_domain', 'domain_pagerank', 'avg_word_length', 'subject_sentiment_score', 'subject_length', 'hour', 'dayofweek', 'stopword_ratio', 'verb_ratio', 'noun_ratio', 'unique_word_ratio', 'exclamation_ratio', 'caps_ratio', 'question_ratio', 'reading_ease', 'external_domain', 'domain_type_Reddit', 'includes_body', 'author_unseen', 'domain_unseen']
...
def get_high_corr_pairs(df: pd.DataFrame, corr_threshold = 0.5):
    ...
    high_corr_pairs = high_corr_pairs[high_corr_pairs["Correlation"].abs() > corr_threshold]
...
def main():
    ...
    ap.add_argument("--corr", default="0.5", help="Correlation threshold above which to remove highly correlated pairs. Default 0.5.")
    ...
    if (args.corr > 1.0) or (args.corr < 0.0):
        raise ValueError(f"{args.corr} not a valid correlation threshold. Must be between 0 and 1.")
    ...
    for col in ["author", "domain"]:
        ...
        encoder_maps[col] = {
            "label_to_code": {
                label: i+1 for i, label in enumerate(classes)
            },
            "unknown_code": 0,
        }
        ...
        for k, df in data.items():
            df[f"{col}_freq"] = df[col].astype(str).map(freq).fillna(0.0).astype("float32")
    ...
    cols_to_include = [x for x in data["train"].columns if ((x not in COLS_TO_REMOVE) & (x not in TARGET_Y_COLS))]
    ...
    for col, mapping in encoder_maps.items():
        for label, code in mapping["label_to_code"].items():
            encoder_rows.append(
                {"column": col, "label": label, "code": code}
            )
    ...
    run_info = {
        ...
        "threads": pd.DataFrame.from_dict(model_info, orient="index", columns=['value']),
        "features": pd.DataFrame([x for x in new_cols if 'svd' not in x], columns=["feature"]),
